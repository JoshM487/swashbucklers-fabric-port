...
public final class HpmEntities {
    ...
    /*
     * Original 2.6.6B behaviour recovered from the NeoForge bytecode:
     * - small ships turn 3 deg/tick; corvette turns 1 deg/tick
     * - world ship-speed multiplier is 0.8
     * - per-ship max-speed values are preserved below
     * - passenger attachment is generic PathfinderMob top-centre, with the exact
     *   vertical offsets used by the original classes (-0.4 swashbucklers, -0.5 cutters)
     * - raft/upgraded/cutters had inventories; swashbuckler/corvette did not
     */
    public static final EntityType<ChestBoat> RAFT = chestBoat(
            "raft", 2.0f, 0.4f, () -> HpmItems.RAFTITEM,
            0.3, 0.03, 0.45, 3.0f, 0.40);

    public static final EntityType<Boat> SWASHBUCKLER = boat(
            "swashbuckler", 1.5f, 0.45f, () -> HpmItems.SWASHBUCKLERITEM,
            0.3, 0.03, 0.60, 3.0f, 0.05);
    ...
    public static final EntityType<ChestBoat> SWASHBUCKLER_UPGRADED = chestBoat(
            ...
            0.3, 0.03, 0.70, 3.0f, 0.05);

    // The original Cutter speed-setting procedure compared against 0.65 but assigned
    // 0.55; preserve the actual assigned value rather than "fixing" that quirk.
    public static final EntityType<ChestBoat> CUTTER = chestBoat(
            "cutter", 1.6f, 0.7f, () -> HpmItems.CUTTERITEM,
            0.525, 0.05, 0.55, 3.0f, 0.20);
